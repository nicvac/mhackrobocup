...
/*
  ...
  if (!onLine)
  {
    ...
    if(contatoreTempo > 100 && tratteggio){
      if(_lastPosition > 3600){
        return 8000;
      }else if(_lastPosition < 3400){
        return 8000;
      }else{
        return 8000;
      }
...
/*
  if(sommaValori / 8 > 520){
    antiVerde = true;
  }*/

  if(!antiVerde){
    if((sensorValues[0] > 210 && sensorValues[0] < 550 && sensorValues[7] > 210 && sensorValues[7] < 550
        && sensorValues[3] > 650 && sensorValues[4] > 650)){
      inversione = true;
      return 9000;
    }else if ((sensorValues[0] > 210 && sensorValues[0] < 550 && sensorValues[3] > 650 && sensorValues[4] > 650)){
      return 10000;
      antiVerde = true;
    }    
  }
  ...
  lastMeaZero = sensorValues[0];
  lastMeaSeven = sensorValues[7];

  if(lastMeaZero > 180 || lastMeaSeven > 180){
    contatoreTempo = 0;
  }else{
    contatoreTempo++;
  }
  ...
  if(lastMeaZero > 180 && lastMeaSeven < 180){
    ultimoEstremo = 0;
  }else if(lastMeaZero < 180 && lastMeaSeven > 180){
    ultimoEstremo = 1;
  }
