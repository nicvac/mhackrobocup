...
class QTRSensors
{
  public:
    ...
    QTRSensors() = default;
  bool ultimoEstremo = 0;
  bool tratteggio = true;
  bool antiVerde = false;
    ~QTRSensors();
  ...
  private:
    ...
    uint32_t contatoreTempo = 0;            //variabile aggiunta
    uint16_t lastMeaZero, lastMeaSeven;     //variabili aggiunte

    uint16_t valoreVerdeMin = 210;
    uint16_t valoreVerdeMax = 600;

    uint16_t contInversione = 0;
