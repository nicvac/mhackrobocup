...
void setup() {
  Serial.begin(115200);
  qtr.setTypeRC();
  qtr.setSensorPins((const uint8_t[]){11, 12, 27, 22, 23, 24, 25, 26}, SensorCount);
  qtr.setEmitterPin(2);
...
inline float gradi2Eng(float gradiCent){ return gradiCent * 160.0 / 90.0; }

inline float Eng2Gradi(float gradiEng){ return gradiEng * 90.0 / 160.0; }
